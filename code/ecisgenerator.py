...
class EcisGenerator:
    ...
    def write_down(self, beam: Nuclei, target: Nuclei, energy: float, xsections: tuple[list[float]]) -> str:
        ...
        if beam.name not in directories:
            os.mkdir(self.__path + f"\\{beam.name}")

        generated_file = self.__path + f"\\{beam.name}\\{target.name}+{beam.name}_{round(energy, 2)}_in.txt"
    ...
    def write_xsections(self, xsections: tuple[list[float]]) -> str:
        ...
        for i in range(n):
            ...
            content += angle_str.ljust(10) + xsec_str.ljust(10) + unc_str.ljust(10) + "\n"
